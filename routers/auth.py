...
SECRET_KEY = "This_is_a_secret_key_for_security"
ALGORITHM = "HS256"
...
def get_password_hash(pss):
    return bcrypt_context.hash(pss)


def verify_password(plain_pss, hashed_pss):
    return bcrypt_context.verify(plain_pss, hashed_pss)
...
def authenticate_user(username: str, password: str, db):
    ...
    if not user:
        return False
    if not verify_password(password, user.hashed_pss):
        return False
    return user
...
async def register_user(
    ...
    user_model.hashed_pss = hash_password
